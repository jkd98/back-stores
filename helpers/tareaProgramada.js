 ...
 */
export const alertaStockMinimo = scheduleJob('0 50 16 * * *', async () => {
    console.log("Tarea programada")
    try {
        const productos = await Producto.findAll({ where: { borrado: false } });
        const productosFaltantes = productos.filter(p => p.stock_actual < p.stock_minimo);
        console.log(productosFaltantes);
        emailAlerta({productos:productosFaltantes});
    } catch (error) {
        console.log(error);
